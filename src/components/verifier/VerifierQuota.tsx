...
export function VerifierQuota({ t, quotaUsed }: { t: (k: string) => string; quotaUsed: number }) {
  ...
  return (
    <div>
      ...
      <div className="portal-panel max-w-lg p-8 rounded-2xl border" style={{ borderColor: 'var(--ct-border)', background: 'var(--ct-surface)' }}>
        ...
        <div className="h-3 rounded-full overflow-hidden mb-6" style={{ background: 'var(--ct-border)' }}>
          <div className={`h-full rounded-full transition-all ${pct >= 90 ? '' : 'portal-quota-fill'}`} style={{ width: `${pct}%`, background: pct >= 90 ? '#ef4444' : undefined }} />
        </div>

        {pct >= 90 && (
          <button className="portal-cta w-full py-3 text-sm font-semibold rounded-xl transition-all hover:opacity-80">
      ...
      <div className="portal-panel mt-8 p-6 rounded-2xl border" style={{ borderColor: 'var(--ct-border)', background: 'var(--ct-surface)' }}>
